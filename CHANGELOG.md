...
**1. モバイル版「もしかして今起きてる？」テロップバーの2段表示化**

モバイル画面（sm 未満）でのテロップバー表示を大幅に改善。従来の1行横スクロール方式から2段レイアウトへ変更し、スマートフォンでの視認性と操作性を向上。

**修正前（モバイル：1行レイアウト）:**
```
[🚨 もしかして今起きてる?] [事故 〇〇駅...] [通信障害 △△...] ...
```
→ 横スクロール必要、テキスト小さい、タップしづらい

**修正後（モバイル：2段レイアウト）:**
```
┌────────────────────────────┐
│ 🚨 もしかして今起きてる？  (0件) │  ← 1段目：ヘッダー
├────────────────────────────┤
│ [事故] 〇〇駅周辺の事故    │  ← 2段目：コンテンツ
│ [通信障害] △△での通信...  │
└────────────────────────────┘
```

**修正内容（IncidentAlert.tsx）:**
- `flex flex-col sm:flex-row` で、モバイルは flex-col（2段縦配置）、PC は flex-row（1段横配置）
- ヘッダー部分：🚨 + ラベル + 件数バッジ（モバイル）
- コンテンツ部分：速報チップまたはメッセージを2段目に表示
- PC 表示時は従来通りの1行レイアウトを維持

**視認性向上:**
- ✅ バッジフォントを `text-xs` に統一（モバイルで読みやすく）
- ✅ ゴシック太字 `font-bold` でカテゴリを強調
- ✅ 最小高さ `min-h-[32px]` で各行の高さを確保
- ✅ 横スクロール時の余白調整（`pb-1` を `sm:pb-0` で PC のみ削除）

---

**2. 地図移動後の「選択中の現場ピンに戻る」ボタン追加**

ユーザーが現場ピンを選択後、地図をドラッグ・ズームした場合に、選択中のピンに素早く戻れるフローティングボタンを実装。

**実装内容:**

**新コンポーネント: `ReturnToPinButton`（Map.tsx 内）**
```typescript
// 動作ロジック:
1. 選択中のピン座標と地図中心の距離を監視
2. 距離 > 0.1 度（約 11km）でボタン表示
3. クリック/タップで flyTo でアニメーション移動
4. ピン選択解除時はボタン非表示
```

**UI 仕様:**
- **位置:** マップ右下（`bottom-20 right-4`）
- **デザイン:** 白背景、薄い枠線、影（`shadow-lg`）
- **テキスト:**
  - PC: 「📍 選択中の現場に戻る」（フル表示）
  - モバイル: 「📍 戻る」（省略表示）
- **アニメーション:** 1.5 秒かけて滑らかに移動

**Props 追加:**
- `Map.tsx`: Props に `selectedPin?: Pin | null` を追加
- `app/page.tsx`: Map 呼び出し時に `selectedPin={selectedPin}` を渡す
...
速報テロップバー（「もしかして今起きてる？」）を常時表示するように変更。速報データが 0 件の場合でも、テロップバー枠組みを画面上に固定して表示し、「現在、検出された速報はありません」というメッセージを静かに表示。ユーザーが速報の有無を一目で判別できるようにしました。
...
return (
  <div className="... min-h-[44px]">
    ...
    <div className="... flex items-center">
      <span className={`... ${hasIncidents ? "animate-pulse" : ""}`}>🚨</span>
      <span>もしかして今起きてる？</span>
    </div>
...
**高さの一貫性:**
- データあり時：`min-h-[44px]` で標準表示
- データ 0 件時：同じ `min-h-[44px]` で一貫性を保つ
- **スクロールバー**: `overflow-hidden` で常に排除

**脈動アニメーションの調整:**
- ✅ データあり：🚨 アイコンが脈動（`animate-pulse`）して注視を引く
- ✅ データ 0 件：🚨 アイコンが静止（アニメーションなし）

#### 3. ユーザー体験の向上

| 状況 | 表示内容 | 備考 |
|-----|--------|------|
| **速報データあり** | テロップバー + チップ表示 | 🚨 アイコン脈動 |
| **速報データ 0 件** | テロップバー + メッセージ | 🚨 アイコン静止 |
| **読み込み中** | テロップバー + メッセージ | 読み込み完了まで表示 |

**メリット:**
- ✅ 常に同じ空間を占有（レイアウト安定性）
- ✅ ユーザーが速報の有無を常に認識（UX 向上）
- ✅ 「速報がない = 平穏」の視覚的フィードバック
- ✅ データ更新時の急な表示/非表示の切り替えがない
