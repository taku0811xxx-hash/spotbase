...
## [2026-08-19] - ジオコーディング精度向上（「新宿駅」→「川崎」バグ修正）

### [修正]
- **ジオコーディング API の検索精度を大幅向上**
  - `lib/geocode.ts` の `geocodeQuery()` 関数を改善
  - **追加パラメータ**：
  ...
  - **ユーザーエージェント設定**：Nominatim の利用ポリシー遵守

- **動作確認スクリプト追加**
  - `scripts/test-geocoding.mjs`：主要駅（新宿・東京・渋谷・品川・横浜）の動作確認
  - すべてのテストキーワードで正確な座標・住所名を返却 ✅

### [検証完了]
✅ 「新宿駅」→ 正確に東京都新宿区（35.69°, 139.70°）を返却
✅ 「東京駅」→ 東京都千代田区の正しい座標を返却
✅ 「渋谷駅」→ 東京都渋谷区の正しい座標を返却
✅ 「品川駅」→ 東京都港区の正しい座標を返却
✅ 「横浜駅」→ 神奈川県横浜市の正しい座標を返却
✅ 日本国内外の座標範囲チェック（川崎などの誤検出を防止）
✅ ビルド成功（`next build --webpack` - TypeScript エラーなし）

**変更ファイル**:
- `lib/geocode.ts` （geocodeQuery 関数を改善）
- `scripts/test-geocoding.mjs` （新規作成 - 動作確認スクリプト）
